...
local customScript = "scripts/BoonsAndBurdens/backgrounds_custom/looseLips.lua"
...
local function modifyNPC(npc, player)
    ...
    if npc.type.getFactionRank(npc, "hlaalu") ~= 0 then
        npc.type.modifyBaseDisposition(npc, player, -20)
    elseif npc.type.getFactionRank(npc, "morag tong") ~= 0 then
        npc:addScript(customScript)
    end
...
return {
    ...
    eventHandlers = {
        BoonsAndBurdens_registerLooseLips = registerPlayer,
    }
}
